...
export const replaceEach = (string:string, remove:string, replace:string) => {
...
export const underscoreToCamelCase = (string:string) => {
  let result = '';
...
export const formatNumber = (number:number) => Number(number).toLocaleString('da-dk');

export const groupByProp =  <T>(arr: T[], keys: (keyof T)[]): { [key: string]: T[] } => arr.reduce((storage, item) => {
...
export const toPerfectPixel = (sizeInEm:number, viewport:TSizeNames) => {
